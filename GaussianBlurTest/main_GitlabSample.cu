...
void Gaussian(double*, double*, int, int, int, int);
void generateGaussian(vector<double>&, int, int);
void errCatch(cudaError_t);
...
int main() {
	...
	*/
	...
	//Use STB image instead of OpenCV

	/* Ignore error handling, vbytes
	...
	*/
	...
	errCatch(cudaDeviceSynchronize()); ignore errors again til can use vbytes or other*/
